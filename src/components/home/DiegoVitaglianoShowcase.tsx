...
import { MapPin, Calendar, Star, Award, TrendingUp, ChevronRight, Image as ImageIcon } from 'lucide-react';
import diegoData from '../../data/diegoVitagliano.json';

export const DiegoVitaglianoShowcase: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'highlights' | 'gallery'>('overview');
  const [activeImage, setActiveImage] = useState(0);

  return (
    <section className="py-20 bg-ticmc-dark text-white relative overflow-hidden" id="diego-vitagliano">
      {/* Background Pattern */}
            ...
            >
              {activeTab === 'overview' && (
                <div className="space-y-8">
                  <div>
                    <h3 className="text-3xl font-serif font-bold text-white mb-6">{diegoData.successStory.title}</h3>
                    
                    {/* Award Image */}
                    {diegoData.images.award && (
                      <div className="mb-8 rounded-sm overflow-hidden border border-white/10 max-w-md">
                        <img src={diegoData.images.award} alt="Top 50 Pizza Award" className="w-full h-auto" />
                    ...
                    <div className="space-y-6 text-gray-300">
                      <div>
                        ...
                        <p>{diegoData.successStory.challenge}</p>
                      </div>
                      <div>
                        <h4 className="text-ticmc-gold font-bold mb-2 uppercase tracking-wider text-sm">The Solution</h4>
                        <p>{diegoData.successStory.solution}</p>
                      </div>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                    {diegoData.successStory.results.map((result, index) => (
                      <div key={index} className="bg-white/5 p-6 rounded-sm border border-white/5 text-center">
                        <div className="mb-4 text-ticmc-gold flex justify-center">
                          {index === 0 ? <Award className="w-8 h-8" /> : 
              ...
              {activeTab === 'highlights' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {diegoData.highlights.map((item, idx) => (
                ...
                <div className="space-y-8">
                  ...
                  <div className="relative group">
                    <div className="relative aspect-video overflow-hidden rounded-sm border border-white/10 shadow-2xl">
                      <motion.img 
                        key={activeImage}
                        src={diegoData.gallery[activeImage]} 
                        alt="Diego Vitagliano Atmosphere" 
