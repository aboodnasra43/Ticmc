...
import { MapPin, Calendar, Star, Music, Coffee, Wine, ChevronRight, FileText, Image as ImageIcon } from 'lucide-react';
import fairouzData from '../../data/fairouz.json';

export const FairouzShowcase: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'highlights' | 'gallery'>('overview');

  return (
    <section className="py-20 bg-ticmc-green text-white relative overflow-hidden" id="fairouz">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-ticmc-gold flex flex-col items-center gap-4">
            <img src="/assets/fairouz/logo.jpg" alt="Fairouz Logo" className="h-24 w-auto object-contain mb-2 rounded-full border-2 border-ticmc-gold" />
            {fairouzData.profile.name}
          </h2>
          <div className="flex justify-center items-center gap-6 text-gray-300">
            <div className="flex items-center gap-2">
              <MapPin className="w-4 h-4 text-ticmc-gold" />
              <span>{fairouzData.profile.location}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-ticmc-gold" />
              <span>Est. {fairouzData.profile.established}</span>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white/5 p-8 rounded-sm border border-white/10 backdrop-blur-sm">
              <p className="text-lg leading-relaxed text-gray-300 mb-8 font-serif italic">
                "{fairouzData.profile.description}"
              </p>
              ...
              <div className="mt-8 pt-8 border-t border-white/10">
                ...
                <div className="space-y-3">
                  <a 
                    href="/assets/fairouz/presentation.pptx" 
                  ...
                  >
                    ...
                    Concept & Menu Presentation (PPTX)
                  </a>
                </div>
              </div>
            </div>
            ...
            >
              {activeTab === 'overview' && (
                <div className="space-y-8">
                  <div>
                    <h3 className="text-3xl font-serif font-bold text-white mb-6">{fairouzData.successStory.title}</h3>
                    <div className="space-y-6 text-gray-300">
                      <div>
                        <h4 className="text-ticmc-gold font-bold mb-2 uppercase tracking-wider text-sm">The Challenge</h4>
                        <p>{fairouzData.successStory.challenge}</p>
                      </div>
                      <div>
                        <h4 className="text-ticmc-gold font-bold mb-2 uppercase tracking-wider text-sm">The Solution</h4>
                        <p>{fairouzData.successStory.solution}</p>
                      </div>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                    {fairouzData.successStory.results.map((result, index) => (
              ...
              {activeTab === 'highlights' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {fairouzData.highlights.map((item, idx) => (
              ...
              {activeTab === 'gallery' && (
                <div className="space-y-8">
                  <h3 className="text-3xl font-serif font-bold text-white mb-6">Design & Collaboration</h3>
                  
                  <div className="grid grid-cols-1 gap-8">
                    <div className="space-y-4">
                      <h4 className="text-ticmc-gold font-bold uppercase tracking-wider text-sm">Emaar Collaboration</h4>
                      <div className="aspect-video rounded-sm overflow-hidden border border-white/10">
                        <img src={fairouzData.images.collab} alt="Emaar Collaboration" className="w-full h-full object-cover" />
                      </div>
                      <p className="text-gray-400 text-sm italic">Showcasing the strategic partnership with Emaar.</p>
                    </div>

                    <div className="space-y-4">
                      <h4 className="text-ticmc-gold font-bold uppercase tracking-wider text-sm">3D Design Concept</h4>
                      <div className="aspect-video rounded-sm overflow-hidden border border-white/10">
                        <img src={fairouzData.images.design} alt="3D Design" className="w-full h-full object-cover" />
                      </div>
                      <p className="text-gray-400 text-sm italic">Innovative 3D design transforming the intimate space.</p>
                    </div>
                  </div>
                </div>
              )}
